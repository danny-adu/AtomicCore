...
namespace AtomicCore.BlockChain.BscscanAPI
{
    ...
    /// </summary>
    internal static class BscscanCacheProvider
    {
        #region Variable
        ...
        public static void Set(string key, object value, BscscanCacheMode mode = BscscanCacheMode.None, TimeSpan? expriedTime = null)
        {
            if (string.IsNullOrEmpty(key))
                throw new ArgumentNullException(nameof(key));
                ...
                lock (dynamicKey)
                {
                    if (null == ck_value)
                    {
                        using (ICacheEntry cacheEntry = s_businessCache_instance.CreateEntry(key))
                        {
                            ...
                            if (mode == BscscanCacheMode.AbsoluteExpired)
                            {
                                if (null == expriedTime)
                            ...
                            else if (mode == BscscanCacheMode.SlideExpired)
                            {
                                if (null == expriedTime)
                                    throw new ArgumentNullException(nameof(expriedTime));
