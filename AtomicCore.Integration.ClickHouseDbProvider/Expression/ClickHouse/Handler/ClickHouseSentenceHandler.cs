...
namespace AtomicCore.Integration.ClickHouseDbProvider
{
    ...
    internal class ClickHouseSentenceHandler : ExpressionVisitorBase
    {
        ...
        private ClickHouseSentenceHandler(IDbMappingHandler dbMappingHandler)
            : base()
        {
            this._dbMappingHandler = dbMappingHandler;
            this._result = ClickHouseSentenceResult.Create();
        }
        ...
        private readonly ClickHouseSentenceResult _result = null;

        /// <summary>
        /// 最终解析的结果
        /// </summary>
        public ClickHouseSentenceResult Result
        {
            ...
            switch (methodCallExp.Method.Name)
            {
                case "Select":
                    ...
                    if (select_exp != null)
                    {
                        ...
                        {
                            ClickHouseSelectCombinedResult selectFieldResult = ClickHouseSelectCombinedHandler.ExecuteResolver(select_func_lambdaExp, this._dbMappingHandler);
                            //接收需要被查询的字段
                ...
                case "Where":
                    ...
                    if (where_exp != null)
                    {
                        ...
                        if (this._result.IsAvailable())
                        {
                            ...
                            ClickHouseWhereScriptResult result = ClickHouseWhereScriptHandler.ExecuteResolver(where_func_lambdaExp, this._dbMappingHandler, false);
        ...
        /// <returns></returns>
        public static ClickHouseSentenceResult ExecuteResolver(Expression exp, IDbMappingHandler dbMappingHandler)
        {
            //执行表达式解析 要被查询的字段
            ClickHouseSentenceHandler entity = new ClickHouseSentenceHandler(dbMappingHandler);
            entity.Visit(exp);
