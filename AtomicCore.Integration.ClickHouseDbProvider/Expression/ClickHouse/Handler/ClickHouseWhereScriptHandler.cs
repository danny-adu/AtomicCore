...
namespace AtomicCore.Integration.ClickHouseDbProvider
{
    ...
    internal static class ClickHouseWhereScriptHandler
    {
        ...
        /// <returns></returns>
        public static ClickHouseWhereScriptResult ExecuteResolver(Expression exp, IDbMappingHandler dbMappingHandler, bool isFieldWithTableName)
        {
            ClickHouseWhereScriptResult result = ClickHouseWhereScriptResult.Create();
            ...
            {
                //执行解析
                ClickHouseConditionCombinedResult resolver = ClickHouseConditionCombinedHandler.ExecuteResolver(exp, dbMappingHandler, isFieldWithTableName);
                ...
                if (architectureParamNumber > 0)
                {
                    ...
                    while (paramEnumerator.MoveNext())
                    {
                        ...
                        if (ExpressionCalculater.IsExistsParameters(itemExp, out _))
                        {
                            ...
                            ClickHouseConditionNodeResult cur_node = ClickHouseConditionNodeHandler.ExecuteResolver(dbMappingHandler, itemExp, isFieldWithTableName);
