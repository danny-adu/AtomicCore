            ...
            if (model != null)
            {
                ...
                if (setFields.Length > 0)
                {
                    ...
                    foreach (var item in setFields)
                    {
                        ...
                        DbParameter paremter = new SqlParameter(ClickHouseGrammarRule.GenerateParamName(parameterName), dbType);
        ...
        public DbNonRecord Update(Expression<Func<M, bool>> whereExp, M model, string suffix = null)
        {
            ...
            foreach (var item in columns)
            {
                ...
                if (p != null)
                {
                    ...
                    cur_parameter = new SqlParameter(ClickHouseGrammarRule.GenerateParamName(parameterName), parameterVal);
                    parameters.Add(cur_parameter);
                }
            }
            sqlBuilder.Replace(",", " ", sqlBuilder.Length - 1, 1);
