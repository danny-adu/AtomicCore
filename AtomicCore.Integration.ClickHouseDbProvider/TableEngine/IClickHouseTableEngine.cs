...
namespace AtomicCore.Integration.ClickHouseDbProvider
{
        ...
        where M : IDbModel, new()
    {
        ...
        /// <param name="updatePropertys">需要被替换或更新的属性</param>
        /// <returns></returns>
        Task<DbNonRecord> UpdateAsync(Expression<Func<M, bool>> whereExp, Expression<Func<M, M>> updatePropertys);
        ...
        /// <param name="model">需要被整体替换的实体</param>
        /// <returns></returns>
        Task<DbNonRecord> UpdateAsync(Expression<Func<M, bool>> whereExp, M model);
        ...
        /// <param name="enableSqlTransaction">是否启动SQL事务（对于单例调用最好启用，对于外层套用事务的不需要启动）</param>
        /// <returns></returns>
        Task<DbNonRecord> UpdateTaskAsync(IEnumerable<DbUpdateTaskData<M>> taskList, bool enableSqlTransaction = false);
        ...
        /// <param name="deleteExp">删除条件</param>
        /// <returns></returns>
        Task<DbNonRecord> DeleteAsync(Expression<Func<M, bool>> deleteExp);
    }
}
